...
template <typename M>
::testing::AssertionResult compare_floating_point_matrices(const char* a_expr,
                                                           const char* b_expr,
                                                           M& a, M& b) {
    ...
    if (is_equal) {
        ...
        return ::testing::AssertionFailure()
               << a_expr << std::endl
               << a << "and " << b_expr << std::endl
               << b << " are not equal" << std::endl;
    }
}
...
TEST(Inverse, Cholesky) {
    ...
    EXPECT_PRED_FORMAT2(compare_floating_point_matrices, inverse_easy_float_mat,
                        result_easy_float_mat);
    EXPECT_TRUE(inverse_easy_float_mat.is_padding_zero());

    const std::array<float, 9> base_harder_float_array = {
        14, 16, 11, 16, 21, 10, 11, 10, 25,
    };

    const std::array<float, 9> result_harder_float_array = {
        425. / 529, -290. / 529, -71. / 529, -290. / 529, 229. / 529,
        36. / 529,  -71. / 529,  36. / 529,  38. / 529,
    };

    BaseMatrix<float, 3, 3> base_harder_float_mat(&base_harder_float_array[0]);
    BaseMatrix<float, 3, 3> inverse_harder_float_mat;
    BaseMatrix<float, 3, 3> result_harder_float_mat(
        &result_harder_float_array[0]);

    Inverse<float, 3>::inverse(base_harder_float_mat, inverse_harder_float_mat);
    EXPECT_PRED_FORMAT2(compare_floating_point_matrices,
                        inverse_harder_float_mat, result_harder_float_mat);
    ...
    BaseMatrix<double, 3, 3> result_easy_double_mat(
        &result_easy_double_array[0]);

    Inverse<double, 3>::inverse(base_easy_double_mat, inverse_easy_double_mat);
    EXPECT_PRED_FORMAT2(compare_floating_point_matrices,
                        inverse_easy_double_mat, result_easy_double_mat);
    ...
    BaseMatrix<double, 3, 3> base_harder_double_mat(
        &base_harder_double_array[0]);
    BaseMatrix<double, 3, 3> inverse_harder_double_mat;
    BaseMatrix<double, 3, 3> result_harder_double_mat(
        &result_harder_double_array[0]);

    Inverse<double, 3>::inverse(base_harder_double_mat,
                                inverse_harder_double_mat);
    EXPECT_PRED_FORMAT2(compare_floating_point_matrices,
                        inverse_harder_double_mat, result_harder_double_mat);
...
TEST(Inverse, Small) {
    ...
    EXPECT_PRED_FORMAT2(compare_floating_point_matrices, inverse_float_mat,
                        result_float_mat);
    ...
    EXPECT_PRED_FORMAT2(compare_floating_point_matrices, inverse_double_mat,
                        result_double_mat);
